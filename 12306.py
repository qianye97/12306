...
to1=input("请输入终点站:")
#to1="上海"
check_sql1 = "select abbr from stationMessage where station = '{}'".format(start1)
check_sql2 = "select abbr from stationMessage where station = '{}'".format(to1)
resultSet1 = cu.execute(check_sql1)
for row1 in resultSet1:
    start = row1[0]
resultSet2 = cu.execute(check_sql2)
for row2 in resultSet2:
    to = row2[0]
date=input("请输入要查询的乘车日期(如2018-06-01)：")
#date="2018-06-07"
#isstudent=input("买学生票吗(默认成人票)？1买，0不买：  ")
isstudent = 0
if(isstudent==1):
    student="0X00"
else:
...
for i in range(0,len(allcheci)):
    try:
        ...
        if(zy==""):
            zy="无"
        print(code+"\t"+fromname+"\t      "+toname.center(8)+"\t"+stime+"\t\t"+atime+"\t\t  "+str(yz)+"\t  "+str(wz)+"     "+str(zy)+"      "+str(ze)+"\n")
...
#以下进入自动登录部分
#myuser = input("请输入用户名：")
#mypasswd = getpass.getpass("请输入密码： ")
...
while not check:
    ...
    e Gecko) Chrome/38.0.2125.122 Safari/537.36 SE 2.X MetaSr 1.0')
    reqlContent = urllib.request.urlopen(req1).read().decode("utf-8","ignore")
    dict = json.loads(reqlContent)
...
6 (KHTML, like Gecko) Chrome/38.0.2125.122 Safari/537.36 SE 2.X MetaSr 1.0')
        ...
        if(chooseno!="None"):
            ...
            print("请选择要订票的用户的序号，此处只能选择一位哦")
            chooseno=input()
            if(chooseno.isdigit()):
                pass
            else:
                chooseno=input("请重新输入：")
...
tionTrainCode="+thiscode+"&seatType=O&fromStationTelecode="+fromStationTelecode+"&toStationTelecod\
